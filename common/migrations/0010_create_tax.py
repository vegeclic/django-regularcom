...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.rename_table('suppliers_tax', 'common_tax')

        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='suppliers', model='tax').update(app_label='common')

    def backwards(self, orm):
        db.rename_table('common_tax', 'suppliers_tax')

        if not db.dry_run:
            # For permissions to work properly after migrating
            orm['contenttypes.contenttype'].objects.filter(app_label='common', model='tax').update(app_label='suppliers')
